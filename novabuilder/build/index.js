( function( wp ) {
	...
	const { Button, SelectControl, TextControl, PanelBody, PanelRow } = wp.components;
					...
					{ node.type === 'heading' && (
						<PanelRow>
							<SelectControl
								...
								onChange={ ( value ) =>
									onUpdate( { ...node, settings: { ...settings, tag: value } } )
								}
							/>
						</PanelRow>
					) }
					{ [ 'heading', 'text', 'button' ].includes( node.type ) && (
						<PanelRow>
							<TextControl
								...
								value={ settings.text || '' }
								onChange={ ( value ) =>
									onUpdate( { ...node, settings: { ...settings, text: value } } )
								}
							/>
						</PanelRow>
					) }
					{ node.type === 'image' && (
						<PanelRow>
							<TextControl
								...
								value={ settings.url || '' }
								onChange={ ( value ) =>
									onUpdate( { ...node, settings: { ...settings, url: value } } )
								}
							/>
						</PanelRow>
					) }
			...
			const reorder = ( nodes ) => {
				...
				if ( currentIndex === -1 || targetIndex === -1 ) {
					return nodes.map( ( node ) => ( {
						...node,
						children: node.children ? reorder( node.children ) : [],
					} ) );
				}
				const updated = [ ...nodes ];
